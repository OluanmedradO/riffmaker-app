...
function WaveformInner({
  ...
  const heights = useMemo(() => {
    ...
    return levels.map((level) => {
      ...
      return { height: normalized * MAX_H, opacity };
    });
  }, [levels]);
  ...
  const animatedTrackStyle = useAnimatedStyle(() => {
    ...
    const progress = clamp01(Math.max(0, playbackPositionMs.value) / (durationMs || 1));
    const pixelPosition = progress * rawWidth;

    return { transform: [{ translateX: -pixelPosition }] };
  }, [rawWidth, durationMs, levels.length]);

  const animatedMarkersStyle = animatedTrackStyle;
  ...
  return (
    <View style={[styles.wrapper, { backgroundColor: theme.input }]}>
      ...
      <View style={styles.container}>
        {/* BARRAS */}
        <Animated.View style={[styles.track, animatedTrackStyle, { width: rawWidth + containerWidth }]}>
          <View style={{ width: containerWidth / 2 }} />

          <View style={{ flexDirection: "row", alignItems: "center", height: "100%", width: rawWidth }}>
            {heights.map((bar, i) => (
              <View
                key={i}
                style={[
        ...
        <Animated.View
          ...
          style={[styles.markersLayer, animatedMarkersStyle, { width: rawWidth + containerWidth }]}
        >
          {markers.map((marker) => {
            ...
            return (
              <Pressable
                ...
                style={[
                  styles.marker,
                  {
                    left: (containerWidth / 2) + Math.max(0, x - MARKER_W / 2),
                    width: MARKER_W,
                    backgroundColor: theme.primary,
                  },
                ]}
