...
export function searchRiffs(riffs: Riff[], query: string): Riff[] {
  ...
  return riffs.filter(
    (riff) => {
      const nameMatch = riff.name?.toLowerCase().includes(lowerQuery);
      const notesMatch = riff.notes?.toLowerCase().includes(lowerQuery);
      const tuningMatch = riff.tuning?.value?.toLowerCase().includes(lowerQuery);
      const tagsMatch = riff.tags?.some(t => t?.toLowerCase().includes(lowerQuery));
      const markersMatch = riff.markers?.some(m => m?.label?.toLowerCase().includes(lowerQuery));
      return nameMatch || notesMatch || tuningMatch || tagsMatch || markersMatch;
    }
  );
}
